...
class EmployeeForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Filter role field to only show active roles
        if 'role' in self.fields:
            self.fields['role'].queryset = Role.objects.filter(status=True)

        # Filter reporting_manager field to exclude users who do not have a role assigned
        ...
        # For styling, add a 'form-control' class to all form fields
